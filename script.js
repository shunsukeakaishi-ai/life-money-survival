...
function render(){
  ...
  const chips=[`現在月 ${state.month}/${MAX_MONTH}`,`現金 ${state.cash.toLocaleString()}円`,`純資産 ${nw.toLocaleString()}円`,`借金 ${state.debt.toLocaleString()}円`,`生活費 ${livingCost().toLocaleString()}円`,`体力 ${state.hp}`,`ストレス ${state.stress}`,`副業疲労 ${state.sidejobFatigue}`,`本業評価 ${state.mainJobScore}`,`投資残高 ${Math.round(state.investmentBalance).toLocaleString()}円`,`投資タイプ ${state.investmentType?investmentDefs[state.investmentType].label:"なし"}`,`投資連続 ${state.investmentStreak}ヶ月`];
